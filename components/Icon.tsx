...
type IconName = 'home' | 'earn' | 'add' | 'profile' | 'recommendations' | 'heart' | 'heart-filled' | 'share' | 'comment' | 'close' | 'coin' | 'gift' | 'users' | 'check-circle' | 'cog' | 'dashboard' | 'video-camera' | 'currency-dollar' | 'shield-check' | 'pencil' | 'trash' | 'eye' | 'logout' | 'moon' | 'sun' | 'menu' | 'arrow-left' | 'warning' | 'volume-up' | 'volume-off' | 'play' | 'pause' | 'rewind' | 'forward' | 'clipboard';
...
const icons: Record<IconName, React.ReactElement> = {
  ...
  earn: <path strokeLinecap="round" strokeLinejoin="round" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v.01M12 14v1m0-6c-1.11 0-2.08-.402-2.599-1M12 14c-1.657 0-3-.895-3-2s1.343-2 3-2 3-.895 3-2 1.343-2 3-2m0 8c1.11 0 2.08.402 2.599 1M12 14c1.657 0 3 .895 3 2s-1.343 2-3 2-3 .895-3 2 1.343 2 3 2m0-8c-1.11 0-2.08-.402-2.599-1" />,
  ...
  recommendations: <path strokeLinecap="round" strokeLinejoin="round" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />,
  ...
  share: <path strokeLinecap="round" strokeLinejoin="round" d="M8.684 13.342C8.846 13.655 9 14.088 9 14.5c0 1.381-1.119 2.5-2.5 2.5S4 15.881 4 14.5c0-.412.154-.845.316-1.158m5.368-5.368C8.846 7.345 9 6.912 9 6.5c0-1.381-1.119-2.5-2.5-2.5S4 5.119 4 6.5c0 .412.154.845.316 1.158m5.368 5.368l2.88-2.88m-2.88 2.88l-2.88-2.88" />,
  ...
  coin: <path strokeLinecap="round" strokeLinejoin="round" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" />,
  ...
  'currency-dollar': <path strokeLinecap="round" strokeLinejoin="round" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v.01M12 14v1m0-6c-1.11 0-2.08-.402-2.599-1M12 14c-1.657 0-3-.895-3-2s1.343-2 3-2 3-.895 3-2 1.343-2 3-2m0 8c1.11 0 2.08.402 2.599 1M12 14c1.657 0 3 .895 3 2s-1.343 2-3 2-3 .895-3 2 1.343 2 3 2m0-8c-1.11 0-2.08-.402-2.599-1" />,
...
const Icon: React.FC<IconProps> = ({ name, className = 'w-6 h-6' }) => {
  ...
  if (name === 'play' || name === 'pause') {
    return (
       <svg xmlns="http://www.w3.org/2000/svg" className={className} fill={name === 'play' ? "currentColor" : "none"} viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
        {icons[name]}
      </svg>
    )
  }
